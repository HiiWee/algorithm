...
* 10진수를 소프트웨어적으로 표현하는 방법은?
  ...
  * 567,832: S = [2, 3, 8, 7, 6, 5] (덧셈과 곱셈을 해보면 거꾸로 표현하는것이 편리)
* 큰 정수의 덧셈
  * n개의 자릿수(digit) 각각을 더하면서 올림수(carry)를 고려
<br><br>
* 큰 정수의 곱셈 : 단순 무식한(Brute-Force) 방법
  * 초등학교에서 배운 방법은 O(n^2)
* 큰 정수의 곱셈: 분할정복(Divde and Conquer)
  * n개의 자릿수(digit)로 된 숫자를 n/2개의 자릿수로 분할
  * 둘 중 하나의 자릿수는 실링 n/2 나머지는 플로어 n/2가 됨
    * 567,832 = 567 * 10^3 + 832
    * 9,423,723 = 9,423 * 10^3 + 723
    * u = x * 10^m + y  [x, y = n/2 digits  u = n digits]
    * 10의 지수: m [n/2 digits]
*  u*v 를 더 작은 자릿수의 곱셈으로 분할 가능하다.
...
* 큰 정수의 곱셈 알고리즘으로 우리가 한 일은?
  ...
  * Algorithm2.9의 시간복잡도 분석:
    * prod() 함수에서 재귀 호출을 네 번 한다는 것에 주목
    * W(s) = 0, W(n) = 4W(n/2) + cn
    * W(n): O(nlog2 4) = O(n^2)

* 알고리즘 2.9의 효율성 개선
  * 결국 기본적인 무식한 계산과 동일한 세타(n^2)을 가짐
  * 재귀 호출의 횟수(현재4번호출)를 줄일 수 는 없을까?
    * 곱셈을 치환하여 줄임

* prod2()의 시간복잡도는?
  * 재귀 호출의 숫자를 3회로 줄임 : 세타(n^1.58)로 세타(n^2)보다 작음

* 분할 정복을 어떻게 접근하는지가 이번시간의 중요한 목표임
  * 분할 정복법의 대표적인 사례를 살펴봤음(어렵다..)